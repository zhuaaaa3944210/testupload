$(function() {
    ...
    function loadSlider() {
        if (IsPC()) {
            var bannerEl = $('#banner');
            if (bannerEl.size()) {
                bannerEl.find("li").show();
                bannerEl.unslider && bannerEl.unslider({
                    // fluid: true,
                    dots: true,
                    speed: 400
                });
    ...
    function autoSize() {
        var bannerHeight = $(window).height() - $('.index_block_img').height(),
            $index_bg = $('.index_img_bg'),
            $word0 = $('.banner0 .index_img_word div'),
            $word2 = $('.banner2 .index_img_word div');
        // $word1 = $('.banner1 .index_img_word div');
        bannerHeight = Math.max(600, bannerHeight);
        $index_bg.css('height', bannerHeight);
        $word0.css('top', ((bannerHeight - $word0.outerHeight()) / 2) - 50);
        // $word1.css('top', ((bannerHeight - 300) / 2) + 30);
        $word2.css('top', ((bannerHeight - $word2.outerHeight()) / 2) - 80);
    ...
    function imgMove() {
        ...
        $('#img_outer').stop().animate({
            'left': -blockCount * eachLiWidth
        }, 500);
    ...
    console.log("123123");
    $('.block_inner_btn').bind('click', function() {
        if ($(this).hasClass('btn_left')) {
            ...
            if (blockCount == 0) {
                ...
                $('#img_outer').css({
                    'left': -eachLiWidth * (imgLength - 1)
                });
                console.log("1:"+(-eachLiWidth * (imgLength - 1)));
            }
            $('#img_outer').stop().animate({
                'left': -blockCount * eachLiWidth
            }, 500);
            console.log("blockCount:"+blockCount);
            console.log("eachWidth:"+eachLiWidth);
    ...
    if (IsPC()) {
        ...
        $(document).bind('scroll', function() {
            ...
            if (scrollTop <= $('.scroll_section').eq(1).attr('mytop')) {
                ...
                $('.img_bg').css({
                    'background-position': '50% ' + scrollTop + 'px'
                });
                $('.download_outer').css({
                    'bottom': (170 - scrollTop)
                });
                $('.check_btn_outer').css({
                    'bottom': (20 - scrollTop)
                });
                ...
                $('.passenger_bg').css({
                    'background-position': '50% ' + (scrollTop - secTop) + 'px'
                });
            }
        });
    }
    ...
    $('.step_btn_outer a').bind(bindEvent, function() {
        ...
        $(this).addClass('active').siblings().removeClass('active');
        $('.step_btn_bg').stop().animate({
            'left': ((thisWidth - 2) * thisIndex + 5)
        }, 300);
    ...
    if (IsIE()) {
        $('menu .spe_li').bind('click', function(ev) {
            ev.stopPropagation();
            if (!$(this).attr('isclick')) {
                $(this).attr('isclick', 'true').children('.sec_menu').animate({
                    'height': 65
                }, 300);
            } else {
                $(this).removeAttr('isclick').children('.sec_menu').animate({
                    'height': 0
                }, 300);
            }
        });
   ...
   /* $('#bg_img_down,.bg_img_down').bind('click', function() {
    ...
    $("#banner").on(bindEvent, ".index_img_bg", function() {
        var it = $(this);
        if (it.is(".banner1")) {
            location.href = "http://static.udache.com/gulfstream/webapp/pages/udache-driver.html";
            return;
        }

        if (it.is(".banner2")) {
            location.href = "http://static.xiaojukeji.com/pinche/pages/website-didialift.html";
            return;
        }
    });
    ...
    console.log("你创造世界的舞台：：http://job.didichuxing.com/");
});
